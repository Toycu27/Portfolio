...
function validate(Array &$response, String $name, String $email, String $message):bool {
    if (strlen($name) < 3) {
        ...
        = '<div class="message warning">Bitte geben sie einen Namen an.</div>';
    }

    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $response['messages']['warning'][] 
        = '<div class="message warning">Bitte geben sie eine gültige E-Mail Adresse an.</div>';
    }

    if (strlen($message) < 3) {
        $response['messages']['warning'][] 
        = '<div class="message warning">Bitte geben sie eine Nachricht ein.</div>';
    }
...
function submit(Array &$response, String $name, String $email, String $message):bool {
    ...
    if(mail("webmaster@toycu.net", $name, $message, $headers)) {
        ...
        = '<div class="message success">Vielen Dank ihre Nachricht wurde erfolgreich verschickt.</div>';
    } else {
        $response['messages']['warning'][] 
        = '<div class="message warning">Es gibt ein Mailserver Problem. Bitte benutzen sie eine alternative Kontaktmöglichkeit.</div>';
    }
